...
public class RegistroPeliculaCategoria {
     ...
     //buscar Película por categoria nombre 
        ...
        try {
            ...
            String sql = "SELECT p.codigo, p.nombre, p.formato4K, c.descripcion, p.precio, c.id FROM peliculas AS p, categoria AS c "
                    + "WHERE p.id_categoria = c.id AND upper(c.descripcion)= upper(?) ";
            ...
            System.out.println("Error SQL al consultar Película por nombre" + e.getMessage());
        } catch (Exception e) {
            System.out.println("Error al consultar Película por nombre" + e.getMessage());
        }
    ...
    //buscar Película por nombre 
        ...
        try {
            ...
            String sql = "SELECT p.codigo, p.nombre, p.formato4K, c.descripcion, p.precio, c.id FROM peliculas AS p, categoria AS c "
                    + "WHERE p.id_categoria = c.id AND p.nombre LIKE ? ";
